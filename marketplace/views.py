...
from associate.models import TenantAppInstallation


def marketplace_home(request):
    """
    A professional marketplace view showing available vs installed apps.
    """
    # Get all active apps from the public registry
    available_apps = GlobalApp.objects.filter(is_active=True)

    # Get what THIS tenant has already installed
    installed_ids = TenantAppInstallation.objects.filter(
        is_active=True
    ).values_list('global_app_id', flat=True)
...
def install_app(request, app_id):
    ...
    exists = TenantAppInstallation.objects.filter(global_app_id=app_id).exists()

    if not exists:
        # 2. In a real app, handle payment processing here

        # 3. Create the installation record in the Tenant Schema
        TenantAppInstallation.objects.create(
            global_app_id=global_app.id,
            is_active=True
        )
        messages.success(request, f"{global_app.name} has been installed successfully!")
    else:
        messages.info(request, f"{global_app.name} is already active.")
